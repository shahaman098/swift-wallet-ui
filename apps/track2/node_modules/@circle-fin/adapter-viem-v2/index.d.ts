 ...
 * a ViemAdapter instance using the `createAdapterFromPrivateKey` factory function.
 *
 ...
 * const defaultAdapter = createAdapterFromPrivateKey({
 *   privateKey: '0x...'
 * })
 *
 * const defaultAdapter1 = createAdapterFromPrivateKey({
 *   privateKey: '1234...abcd', // Without prefix
 * })
 *
 * // Developer-controlled adapter (requires addresses)
 * const devAdapter = createAdapterFromPrivateKey({
 *   privateKey: '0x...',
 *   capabilities: { addressContext: 'developer-controlled' }
 * })
 *
 * const devAdapter1 = createAdapterFromPrivateKey({
 *   privateKey: '1234...abcd', // Without prefix
 *   capabilities: { addressContext: 'developer-controlled' }
 * })
 *
 * // User-controlled adapter (addresses forbidden)
 * const userAdapter = createAdapterFromPrivateKey({
 *   privateKey: '0x...',
 *   capabilities: { addressContext: 'user-controlled' }
 * })
 *
 * const userAdapter1 = createAdapterFromPrivateKey({
 *   privateKey: '1234...abcd', // Without prefix
 *   capabilities: { addressContext: 'user-controlled' }
 * })
 * ```
 */
declare function createAdapterFromPrivateKey<const TCapabilities extends Partial<AdapterCapabilities> = object>(params: CreateAdapterFromPrivateKeyParams<TCapabilities>): ViemAdapter<InferAdapterCapabilities<TCapabilities>>;

/**
 * Parameters for creating a ViemAdapter from an EIP1193 provider.
 *
 * This interface defines the configuration options available when creating
 * a ViemAdapter instance using the `createAdapterFromProvider` factory function.
 *
 * @typeParam TCapabilities - The adapter capabilities type for compile-time address validation
 */
interface CreateAdapterFromProviderParams<TCapabilities extends Partial<AdapterCapabilities> = object> {
    /**
 ...
 * ```typescript
 * // Developer-controlled adapter (requires addresses)
 * const devAdapter = await createAdapterFromProvider({
 *   provider: window.ethereum,
 *   capabilities: { addressContext: 'developer-controlled' }
 * })
 *
 * // User-controlled adapter (addresses forbidden)
 * const userAdapter = await createAdapterFromProvider({
 *   provider: window.ethereum,
 *   capabilities: { addressContext: 'user-controlled' }
 * })
 *
 * // Default adapter (user-controlled)
 * const defaultAdapter = await createAdapterFromProvider({
 *   provider: window.ethereum
 * })
 * ```
 */
declare function createAdapterFromProvider<const TCapabilities extends Partial<AdapterCapabilities> = object>(params: CreateAdapterFromProviderParams<TCapabilities>): Promise<ViemAdapter<InferAdapterCapabilities<TCapabilities>>>;

export { Blockchain, ViemAdapter, buildEIP2612TypedData, computeDefaultDeadline, createAdapterFromPrivateKey, createAdapterFromProvider, parseSignature, resolveChainIdentifier };
export type { ChainIdentifier, CreateAdapterFromPrivateKeyParams, CreateAdapterFromProviderParams, EIP2612Message, EIP2612NonceFetcher, EIP2612Options, PermitStandardName, Signature, TypedData, TypedDataField, ViemAdapterOptions };
