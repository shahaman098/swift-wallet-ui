...
import AllocationPieChart from "@/components/treasury/AllocationPieChart";
import NextPayoutCountdown from "@/components/treasury/NextPayoutCountdown";
import TreasuryEventLog from "@/components/treasury/TreasuryEventLog";
import ApprovalModal from "@/components/treasury/ApprovalModal";
import {
  Activity,
  AlertTriangle,
  ArrowRightLeft,
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950 text-white">
      ...
      <main className="container mx-auto px-4 py-10 max-w-7xl relative z-10">
        ...
        {/* UI-only additive: Treasury overview row */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <TreasuryOverview
            balance={0}
            allocations={[{ name: "Ops", percent: 40 }, { name: "R&D", percent: 35 }, { name: "Marketing", percent: 25 }]}
            lastStatus={"Idle"}
          />
          <AllocationPieChart data={[{ name: "Ops", value: 40 }, { name: "R&D", value: 35 }, { name: "Marketing", value: 25 }]} />
          <NextPayoutCountdown lastRun={Math.floor(Date.now() / 1000) - 1200} intervalSec={3600} />
        </div>

        {/* UI-only additive: approvals + events */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-8">
          <div className="border rounded-2xl p-4 bg-white/5">
            <div className="text-sm text-muted-foreground mb-2">Approvals</div>
            <ApprovalModal onConfirm={()=>{ /* hook into existing approval endpoint if desired */ }} />
            <div className="text-xs text-muted-foreground mt-2">
              This modal is a UX shell only. Connect it to your existing gateway approval logic.
            </div>
          </div>
          <div className="md:col-span-2">
            <TreasuryEventLog items={[]} />
          </div>
        </div>
