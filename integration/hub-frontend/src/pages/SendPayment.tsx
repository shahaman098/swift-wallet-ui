...
const SendPayment = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      setLoading(true);
      const { data } = await apiClient.post("/payments/send", {
        recipientEmail: recipient,
        amount,
      });

      setTxHash(data?.transfer?.transactionHash ?? null);
      setSuccess(true);
      toast({
        title: "Payment sent",
      ...
      if (isFrontendDemo) {
        // Simulate success so the UI shows progress working in demo mode
        const mockHash = `0x${Math.random().toString(16).slice(2).padEnd(64, "0")}`;
        setTxHash(mockHash);
        setSuccess(true);
        toast({
          title: "Payment sent (demo)",
          description: `$${parseFloat(amount).toFixed(2)} sent to ${recipient}`,
        });
