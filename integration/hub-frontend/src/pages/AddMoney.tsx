...
const AddMoney = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const isFrontendDemo =
        (import.meta as any)?.env?.VITE_FRONTEND_DEMO === "true";
      if (isFrontendDemo) {
        // Simulate a full success flow in demo mode so users can see it working
        const mockMintHash = `0x${Math.random().toString(16).slice(2).padEnd(64, "0")}`;
        setBurnTxHash(null);
        setMintTxHash(mockMintHash);
        setAmountMinted(amount);
        setFinalBalance(amount);
        setSuccess(true);
        toast({
          title: "Money added (demo)",
          description: `Minted ${amount} USDC on Arc.`,
        });
