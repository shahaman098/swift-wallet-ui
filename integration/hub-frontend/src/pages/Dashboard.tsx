...
import GlobalWalletPreview from "@/components/GlobalWalletPreview";
import UpcomingAutomationPreview from "@/components/UpcomingAutomationPreview";
import Track1Snapshot from "@/components/snapshots/Track1Snapshot";
import Track3Snapshot from "@/components/snapshots/Track3Snapshot";
...
import { TrendingUp, Activity, PieChartIcon, Bot, Sparkles, Wallet, ArrowRightLeft, PiggyBank, FileCode } from "lucide-react";
  ...
  const fetchWalletData = useCallback(
    async (options?: { silent?: boolean }) => {
      ...
      try {
        ...
        const numericBalance = parseFloat(balanceRes.data?.balance ?? 0);
        setBalance(Number.isFinite(numericBalance) ? numericBalance : 0);

        const transactions = activityRes.data?.transactions ?? [];
        setTransactions(
          transactions.map((tx: any) => {
            const normalizedType = (tx.type ?? "").toLowerCase();
            const directionType =
              normalizedType.includes("send") || tx.direction === "out"
                ? "send"
                : "deposit";
            return {
              id:
              ...
              recipient:
                directionType === "send"
                  ? tx.to ?? tx.recipient ?? tx.destination
                  : tx.from ?? tx.source,
              date: new Date(tx.timestamp ?? tx.createdAt ?? Date.now()).toLocaleString(),
      ...
      <main className="container mx-auto px-4 py-8 max-w-7xl relative z-10">
        ...
        >
          <div>
            ...
            <p className="text-muted-foreground text-lg">Live balance, recent payments, upcoming automations, bridges, and treasury events — all in one place.</p>
          </div>
          <HelpModal />
        </motion.div>
        ...
        <div className="space-y-8">
          ...
          >
            <div className="flex items-center gap-3 mb-4">
              ...
              />
              <h2 className="text-3xl font-bold text-arc-gradient">Feature Modules</h2>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <Card className="liquid-glass-premium border-0 shadow-xl rounded-3xl overflow-hidden shimmer h-full">
                  ...
                  <CardContent className="pt-6">
                    <p className="text-sm text-muted-foreground mb-4">
                      Access your wallet, view balance, send payments, and track transactions all in one place.
                    </p>
                    <Button
              ...
              >
                <Card className="liquid-glass-premium border-0 shadow-xl rounded-3xl overflow-hidden shimmer h-full">
                  <CardHeader className="border-b border-white/10 bg-gradient-to-br from-accent/5 to-transparent">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <CardDescription className="mt-1">
                          Move USDC across chains using Circle's Bridge Kit on Arc
                        </CardDescription>
                      </div>
                    </div>
                  </CardHeader>
                  <CardContent className="pt-6">
                    <p className="text-sm text-muted-foreground mb-4">
                      Seamlessly bridge assets between Arc Testnet, Base Sepolia, Ethereum Sepolia, and Polygon Amoy.
                    </p>
                    <Button 
                ...
                <Card className="liquid-glass-premium border-0 shadow-xl rounded-3xl overflow-hidden shimmer h-full">
                  <CardHeader className="border-b border-white/10 bg-gradient-to-br from-primary/5 to-transparent">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-xl">Treasury Automation</CardTitle>
                        <CardDescription className="mt-1">
                  ...
                  <CardContent className="pt-6">
                    <p className="text-sm text-muted-foreground mb-4">
                      Set up automated payment schedules, manage treasury organizations, and monitor ML-powered insights.
                    </p>
                    <Button 
              ...
              {/* Smart Contracts Playground Card */}
              <motion.div
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                ...
                <Card className="liquid-glass-premium border-0 shadow-xl rounded-3xl overflow-hidden shimmer h-full">
                  <CardHeader className="border-b border-white/10 bg-gradient-to-br from-accent/5 to-transparent">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <CardTitle className="text-xl">Smart Contracts Playground</CardTitle>
                        <CardDescription className="mt-1">
                  ...
                  <CardContent className="pt-6">
                    <p className="text-sm text-muted-foreground mb-4">
                      {/* TODO: Build Track 1 frontend - Smart Contract demo UI */}
                      Deploy, test, and interact with smart contracts directly from your browser.
                    </p>
                    <Button 
